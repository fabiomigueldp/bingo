...
COLUMN_SHORT_FUNCTIONS = {
    ...
    "I": "formas concretas de violência e discriminação",
...
def load_project_data() -> tuple[dict, dict, list[dict], list[dict]]:
    global COLUMN_FULL_TITLES, COLUMN_SHORT_FUNCTIONS
...
def draw_bingo_card(c: canvas.Canvas, card: dict, x: float, y: float, w: float, h: float) -> None:
    ...
    c.drawString(strip_x + 4.5 * mm, hstrip_y + 5.5 * mm, "Bingo — Violência e discriminação")

    # Sub-metadata — also cream (white), smaller, same color to maintain two-tone elegance
    c.setFont(FONTS["sans_bold"], 5.8)
    c.drawString(strip_x + 4.5 * mm, hstrip_y + 2.0 * mm, "Encontro 04")
...
def build_bingo_cards(terms: list[dict]) -> Path:
    OUT_DIR.mkdir(parents=True, exist_ok=True)
    path = OUT_DIR / "cartelas_bingo_24_a4.pdf"
    cards = generate_bingo_cards(terms)

    c = canvas.Canvas(str(path), pagesize=A4)
    c.setTitle("Cartelas do bingo — Violência e discriminação")
    margin_x = 10 * mm
    margin_y = 9 * mm
    gap = 7 * mm
...
def build_call_deck(terms: list[dict], calls: list[dict]) -> Path:
    ...
    c.setTitle("Baralho de chamadas — Bingo Violência e discriminação")
...
def build_control_sheet(terms: list[dict]) -> Path:
    ...
    grouped = terms_by_column(terms)
    c = canvas.Canvas(str(path), pagesize=landscape(A4))
    c.setTitle("Folha de controle de chamadas — Bingo Violência e discriminação")
...
def build_team_rule_cards() -> Path:
    ...
    c = canvas.Canvas(str(path), pagesize=A4)
    c.setTitle("Cartões de regras para equipes — Bingo Violência e discriminação")
...
def build_facilitator_guide(terms_data: dict, calls_data: dict) -> Path:
    ...
    doc = SimpleDocTemplate(
        ...
        topMargin=18 * mm,
        bottomMargin=16 * mm,
        title="Guia de condução — Bingo Violência e discriminação",
    )
    styles = guide_styles()
    story = []
    frame_w = doc.width
    ...
    story.append(Paragraph("Bingo — Violência e discriminação · Encontro 04", styles["subtitle"]))
    story.append(
        guide_band(
            "Critério central",
    ...
    story.append(
        guide_card_grid(
            [
                ...
                (
                    "Fechamento",
                    "Violência e discriminação começam quando alguém deixa de ser visto como pessoa. A fé cristã começa pelo contrário: cada pessoa foi criada à imagem de Deus.",
                    "Script",
                ),
            ],
            frame_w,
...
def build_explanations_letter(terms: list[dict], calls: list[dict], explanations_data: dict) -> Path:
    ...
    doc = BaseDocTemplate(
        ...
        title="Carta de explicações rápidas — Bingo Violência e discriminação",
    )
...
def merge_pdfs(paths: list[Path]) -> Path:
    out = OUT_DIR / "kit_impressao_bingo_violencia_discriminacao.pdf"
    writer = PdfWriter()
    for path in paths:
...
def main() -> None:
    ...
    cartelas = build_bingo_cards(terms)
    ...
    for path in [cartelas, chamadas, explicacoes, regras, controle, guia, kit]:
