...
GAME_DATA_PATH = ROOT / "src" / "data" / "game-data.json"
...
def app_text() -> tuple[str, str]:
    if not GAME_DATA_PATH.exists():
        return "Bingo", "Jogo catequético"

    data = json.loads(GAME_DATA_PATH.read_text(encoding="utf-8"))
    title = data.get("title", "Bingo")
    subtitle = data.get("subtitle") or re.sub(r"^bingo\s*[-—]\s*", "", title, flags=re.IGNORECASE).strip()
    return "Bingo", subtitle or title
...
def make_screenshot(path: Path, width: int, height: int, tablet: bool = False) -> None:
    app_title, app_subtitle = app_text()
    ...
    text_center(draw, (width // 2, int(height * 0.10)), app_title, INK, int(width * (0.095 if tablet else 0.13)), True)
    text_center(
        draw,
...
def make_splash(path: Path, width: int, height: int) -> None:
    ...
    text_center(draw, (width // 2, int(height * 0.56)), app_title, INK, int(width * 0.09), True)
    text_center(draw, (width // 2, int(height * 0.605)), app_subtitle, MUTED, int(width * 0.032), True)
    img.save(path, quality=92)
