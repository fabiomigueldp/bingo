const DEFAULT_COLUMN_ORDER = ["B", "I", "N", "G", "O"];
...
export function createGameModel(data) {
  const configuredColumnOrder = data.columns?.map((column) => column.key);
  const COLUMN_ORDER = configuredColumnOrder?.length ? configuredColumnOrder : DEFAULT_COLUMN_ORDER;
  const cardById = Object.fromEntries(data.cards.map((card) => [card.id, card]));
  const cardByCode = Object.fromEntries(data.cards.map((card) => [card.code, card]));
  const boardByNumber = Object.fromEntries(data.boards.map((board) => [board.number, board]));
  const columnByKey = Object.fromEntries(data.columns.map((column) => [column.key, column]));

  function lineLabel(line) {
    if (line.type === "horizontal") return `Linha ${line.index}`;
    if (line.type === "vertical") return `Coluna ${COLUMN_ORDER[line.index - 1] || line.index}`;
    if (line.index === 1) return "Diagonal principal";
    return "Diagonal secundária";
  }

  function normalizeManifestLine(line) {
    return {
      id: `${line.type}-${line.index}`,
      kind: line.type,
      label: lineLabel(line),
      cells: line.cells.map(([row, col]) => [row - 1, col - 1])
    };
  }

  const fallbackLineDefinitions = [
  ...
  const lineDefinitions = data.lineModel?.length
    ? data.lineModel.map(normalizeManifestLine)
    : fallbackLineDefinitions;
  ...
  function createGame(activeBoardNumbers, drawMode = DRAW_MODES.APP) {
    return {
  ...
  function markedIdsFromDrawn(drawnIds) {
  ...
  function getCompletedLines(board, drawnIds) {
  ...
  function getBoardProgress(board, drawnIds) {
    const marked = markedIdsFromDrawn(drawnIds);
  ...
  function collectReadyAlerts(game, drawnIds) {
  ...
  function recomputeNotifications(activeBoardNumbers, drawnIds) {
    const notified = {};
  ...
  function drawCardById(game, cardId) {
  ...
  function dismissAlertBatch(game, drawIndex) {
    return {
      ...game,
  ...
  function dismissBoardAlert(game, boardNumber, drawIndex) {
    return {
      ...game,
  ...
  function recordValidatedWin(game, boardNumber, lineId) {
    return {
      ...game,
      validatedWins: [
        ...game.validatedWins,
